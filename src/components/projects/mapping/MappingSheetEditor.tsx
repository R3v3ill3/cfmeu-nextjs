...
export type MappingSheetEditorDataSource =
  | { type: 'token'; token: string }
  | { type: 'project'; projectId: string }

interface MappingSheetEditorProps {
  dataSource: MappingSheetEditorDataSource;
  variant?: 'standalone' | 'embedded';
  onClose?: () => void;
}

export function MappingSheetEditor({
  dataSource,
  variant = 'standalone',
  onClose,
}: MappingSheetEditorProps) {
  const queryClient = useQueryClient();

  const dataKey = dataSource.type === 'token' ? dataSource.token : dataSource.projectId;
  const dataUrl =
    dataSource.type === 'token'
      ? `/api/public/form-data/${dataSource.token}`
      : `/api/projects/${dataSource.projectId}/mapping-sheet`;
  const submitUrl =
    dataSource.type === 'token'
      ? `/api/public/form-data/${dataSource.token}`
      : `/api/projects/${dataSource.projectId}/mapping-sheet`;
  ...
  const { data: publicData, isLoading, error } = useQuery<PublicFormData>({
    queryKey: ['mapping-sheet-form', dataSource.type, dataKey],
    enabled: !!dataKey,
    queryFn: async () => {
  ...
  const containerClass = isEmbedded ? 'bg-gray-50' : 'min-h-screen bg-gray-50';
  const centeredClass = isEmbedded
    ? 'bg-gray-50 flex items-center justify-center py-8'
