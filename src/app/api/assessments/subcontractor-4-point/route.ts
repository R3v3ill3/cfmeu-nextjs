...
const SubcontractorAssessmentSchema = z.object({
  ...
  assessment_basis: z.enum(['payroll_review', 'site_observation', 'union_knowledge', 'contractor_interview', 'document_review', 'other']).default('site_observation'),
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create the assessment using the new schema
    const assessmentData = {
      employer_id: validatedData.employer_id,
      project_id: validatedData.project_id || null,
      assessor_id: user.id,
      assessment_date: new Date().toISOString().split('T')[0],
      usage_rating: validatedData.usage_rating,
      subcontractor_count: validatedData.subcontractor_count || null,
      subcontractor_percentage: validatedData.subcontractor_percentage || null,
      assessment_basis: validatedData.assessment_basis,
    ...
    const { data: assessment, error } = await supabase
      .from('subcontractor_assessments')
    ...
    try {
      await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/ratings/calculate-4-point-employer-rating`, {
        method: 'POST',
        headers: {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const assessmentBasis = searchParams.get('assessment_basis')
    ...
    if (assessmentBasis) {
      query = query.eq('assessment_basis', assessmentBasis)
    }
    if (confidenceLevel) {
