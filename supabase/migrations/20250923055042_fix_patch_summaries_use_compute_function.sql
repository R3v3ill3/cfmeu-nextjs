-- Fix get_patch_summaries_for_user to use compute_patches_for_lead_organizer function
-- This ensures both live and draft lead organisers work correctly using proven patches selector logic
...
BEGIN
  ...
  -- 🔧 FIX: Use compute_patches_for_lead_organizer function for both live and draft leads
  patch_ids_filter := NULL;

  IF effective_role = 'admin' THEN
    IF lead_filter IS NOT NULL THEN
      -- Use the proven patches selector logic via compute function
      patch_ids_filter := compute_patches_for_lead_organizer(lead_filter);
...
COMMENT ON FUNCTION public.get_patch_summaries_for_user IS
  'Role-aware patch summaries using compute_patches_for_lead_organizer for both live and draft leads';
