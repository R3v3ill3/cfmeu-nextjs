-- Fix: Admin viewing coordinator-specific metrics should respect patch filtering
-- This addresses the issue where all coordinators show identical metrics
...
BEGIN
  ...
  -- 🔧 FIX: Modified logic to respect patch filtering for admin coordinator queries
  patch_ids_filter := NULL;
  IF effective_role = 'admin' THEN
    -- Admin has full access but should respect explicit patch filtering when provided
    patch_ids_filter := NULL; -- full access by default
  ELSIF effective_role = 'lead_organiser' THEN
    patch_ids_filter := NULL; -- full access
  ELSE
    -- Organiser-specific logic (unchanged)
  ...
  -- 🔧 FIX: Always respect requested patch IDs when explicitly provided
  IF patch_ids_filter IS NULL THEN
    final_patch_ids := requested_patch_ids; -- Use requested patches directly
  ...
  WITH filtered_projects AS (
    ...
    WHERE p.organising_universe::text = sanitized_universe
      ...
      -- 🔧 FIX: Modified condition to properly handle explicit patch filtering
      AND (
        final_patch_ids IS NULL 
        OR (final_patch_ids IS NOT NULL AND array_length(final_patch_ids, 1) > 0 AND pjs.patch_id = ANY(final_patch_ids))
      )
  ),
  project_builders AS (
    SELECT 
