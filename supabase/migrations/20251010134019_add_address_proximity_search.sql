-- Migration: Add Address Proximity Search
-- Creates function to find projects by address with exact match and proximity search

-- Function to find nearby projects based on coordinates and optional address text
...
COMMENT ON FUNCTION find_nearby_projects(double precision, double precision, text, integer, double precision) IS
  'Finds projects near given coordinates. Returns exact matches first, then closest by distance. Limited to max_distance_km radius (default 100km).';

-- Ensure spatial index exists on job_sites for performance
CREATE INDEX IF NOT EXISTS idx_job_sites_geom
  ON job_sites USING GIST (geom);

-- Create additional index on coordinates for faster lookups
CREATE INDEX IF NOT EXISTS idx_job_sites_lat_lng
