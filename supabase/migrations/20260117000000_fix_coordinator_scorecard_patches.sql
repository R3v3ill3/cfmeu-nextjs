-- Fix coordinator scorecards to include patches for draft lead organisers
-- and live leads with draft-only organiser teams.
--
-- Root cause: The previous migration (20250924091500_patch_summaries_scope_fix.sql)
-- regressed to querying lead_organiser_patch_assignments directly, which only
-- contains live leads. Draft lead organisers (in pending_users table) were excluded.
--
-- Fix: Use compute_patches_for_lead_organiser() function which correctly handles
-- both live AND draft lead organisers.
...
BEGIN
  ...
  -- ==========================================================================
  -- FIX: Use compute_patches_for_lead_organiser() for both live AND draft leads
  -- This function correctly handles:
  --   1. Live lead organisers (profiles table)
  --   2. Draft lead organisers (pending_users table)
  --   3. Live leads with draft organiser teams
  --   4. Draft leads with mixed organiser teams
  -- ==========================================================================
  patch_ids_filter := NULL;

  IF effective_role = 'admin' THEN
    IF lead_filter IS NOT NULL THEN
      -- Use the compute function that handles both live and draft leads
    ...
    END IF;
  ELSIF effective_role = 'lead_organiser' THEN
    -- Use compute function for the lead organiser's patches
  ...
  -- Return empty if no patches found (don't return all patches)
  ...
  -- ==========================================================================
  -- FIX: Include both live AND draft organisers in the name listing
  -- ==========================================================================
  patch_organisers AS (
    SELECT
      ps.id AS patch_id,
...
COMMENT ON FUNCTION public.get_patch_summaries_for_user IS
  'Role-aware patch summaries using compute_patches_for_lead_organiser for both live and draft leads. Includes draft organisers in organiser names.';
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '==========================================================================';
  RAISE NOTICE '✅ COORDINATOR SCORECARD PATCH FIX APPLIED';
  RAISE NOTICE '==========================================================================';
  RAISE NOTICE '';
  RAISE NOTICE 'Changes made:';
  RAISE NOTICE '  1. get_patch_summaries_for_user now uses compute_patches_for_lead_organiser()';
  RAISE NOTICE '     for admin and lead_organiser roles, enabling draft lead support';
  RAISE NOTICE '';
  RAISE NOTICE '  2. patch_organisers CTE now includes draft organisers from pending_users';
  RAISE NOTICE '     where status IN (''draft'', ''invited'')';
  RAISE NOTICE '';
  RAISE NOTICE 'This fixes:';
  RAISE NOTICE '  - Draft lead organisers showing 0 patches in scorecards';
  RAISE NOTICE '  - Live leads with draft-only teams showing incomplete data';
  RAISE NOTICE '  - Missing organiser names for draft/invited organisers';
  RAISE NOTICE '';
END $$;
