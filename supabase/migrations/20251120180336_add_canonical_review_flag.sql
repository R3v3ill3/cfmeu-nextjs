-- Add mark_for_canonical_review flag to employer_aliases table
-- This allows users to explicitly opt-in aliases for canonical promotion review
-- Only aliases with this flag set to true will appear in the canonical_promotion_queue

-- 1. Add the new column with default false
ALTER TABLE public.employer_aliases
  ADD COLUMN IF NOT EXISTS mark_for_canonical_review boolean NOT NULL DEFAULT false;

-- 2. Add index for query performance
CREATE INDEX IF NOT EXISTS idx_employer_aliases_mark_for_canonical_review
  ON public.employer_aliases(mark_for_canonical_review)
  WHERE mark_for_canonical_review = true;

COMMENT ON COLUMN public.employer_aliases.mark_for_canonical_review
  IS 'Explicit opt-in flag for canonical promotion review. Only aliases with this flag set to true will appear in the canonical_promotion_queue.';

-- 3. Update canonical_promotion_queue view to require explicit opt-in
...
WHERE 
  ...
  -- NEW: Require explicit opt-in for canonical review
  AND ea.mark_for_canonical_review = true
  AND (
...
COMMENT ON VIEW public.canonical_promotion_queue 
  IS 'Queue of aliases explicitly marked for canonical name promotion review. Only includes aliases where mark_for_canonical_review = true. Excludes rejected and deferred items.';
