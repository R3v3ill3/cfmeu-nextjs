-- Migration: Fix Address Search Type Casting
-- Fixes the organising_universe enum to text conversion issue
...
COMMENT ON FUNCTION find_nearby_projects(double precision, double precision, text, integer, double precision) IS
  'Finds projects near given coordinates. Returns exact matches first, then closest by distance. Limited to max_distance_km radius (default 100km). Fixed type casting for organising_universe enum.';
