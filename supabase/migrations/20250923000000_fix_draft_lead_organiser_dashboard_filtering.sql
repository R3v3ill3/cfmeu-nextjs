-- Fix get_patch_summaries_for_user to properly handle draft lead organisers
-- This resolves the issue where draft lead organisers were not getting their assigned patches
-- Uses simpler approach: RPC function handles draft lead organizer IDs directly
...
BEGIN
  -- Check if the lead organiser is a draft lead organiser
  ...
  IF p_user_role = 'organiser' THEN
    ...
    SELECT ARRAY_AGG(opa.patch_id) FILTER (WHERE opa.patch_id IS NOT NULL) INTO patch_ids_filter
  ...
  ELSIF p_user_role = 'lead_organiser' THEN
    ...
    SELECT ARRAY_AGG(lopa.patch_id) FILTER (WHERE lopa.patch_id IS NOT NULL) INTO patch_ids_filter
  ...
  ELSIF p_user_role = 'admin' THEN
    ...
    IF p_lead_organiser_id IS NOT NULL THEN
      IF is_draft_lead THEN
        -- Draft lead organiser: Get patches from assigned_patch_ids, filtered to existing patches only
        SELECT ARRAY_AGG(p.id) INTO patch_ids_filter
        FROM pending_users pu
        CROSS JOIN UNNEST(pu.assigned_patch_ids) AS assigned_patch_id
        JOIN patches p ON p.id = assigned_patch_id
      ...
      END IF;
    ELSE
      patch_ids_filter := NULL; -- All patches
    END IF;
  END IF;

  -- If no patches found, return empty result
  IF patch_ids_filter IS NOT NULL AND array_length(patch_ids_filter, 1) = 0 THEN
    RETURN;
  END IF;
  ...
  patch_organisers AS (
    SELECT
      p.id AS patch_id,
      COALESCE(ARRAY_AGG(DISTINCT pr.full_name ORDER BY pr.full_name) FILTER (WHERE pr.full_name IS NOT NULL), ARRAY[]::TEXT[]) AS organiser_names
    FROM relevant_patches p
    LEFT JOIN organiser_patch_assignments opa
      ON opa.patch_id = p.id
...
COMMENT ON FUNCTION public.get_patch_summaries_for_user IS
  'Simplified role-aware patch summaries that handle draft lead organisers by using their assigned_patch_ids directly';
