-- Fix multiple issues in create_project_from_scan
-- 1. Cast site_contact_role from text to enum
-- 2. Only insert subcontractor assignments when trade_type_id is not null (satisfies check_assignment_fields constraint)
...
COMMENT ON FUNCTION create_project_from_scan(uuid, uuid, jsonb, jsonb, jsonb, jsonb, boolean) IS
  'Creates a complete project from a mapping sheet scan. Fixed: (1) site_contact_role casting from text to enum, (2) check_assignment_fields constraint by requiring trade_type_id.';
