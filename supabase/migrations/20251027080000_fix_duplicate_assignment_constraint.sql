-- Fix duplicate assignment constraint violation in create_project_from_scan
-- Use ON CONFLICT to handle duplicate (project_id, employer_id, trade_type_id) combinations
-- This prevents errors when the same employer is added multiple times to the same trade
...
BEGIN
  ...
  BEGIN
    ...
    -- Fixed: Cast role from text to site_contact_role enum
    IF v_job_site_id IS NOT NULL THEN
      ...
      LOOP
        INSERT INTO site_contacts (
          ...
          v_job_site_id,
          (v_contact->>'role')::site_contact_role,  -- Explicit cast to enum
          v_contact->>'name',
    ...
    -- Fixed: Handle duplicate assignments using ON CONFLICT
    ...
    LOOP
      ...
      -- Only insert if we have a valid trade_type_id (required by check constraint)
      -- Check for existing assignment first to avoid unique constraint violation
      IF v_trade_type_id IS NOT NULL THEN
        -- Check if assignment already exists
        IF NOT EXISTS (
          SELECT 1 FROM project_assignments
          WHERE project_id = v_project_id
            AND employer_id = v_employer_id
            AND trade_type_id = v_trade_type_id
            AND assignment_type = 'trade_work'
        ) THEN
...
COMMENT ON FUNCTION create_project_from_scan(uuid, uuid, jsonb, jsonb, jsonb, jsonb, boolean) IS
  'Creates a complete project from a mapping sheet scan. Fixed: (1) site_contact_role casting from text to enum, (2) check_assignment_fields constraint by requiring trade_type_id, (3) duplicate assignment constraint using ON CONFLICT DO NOTHING.';
