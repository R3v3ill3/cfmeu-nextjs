-- Migration: Add statement timeout to find_nearby_projects
-- Prevents the RPC from hanging indefinitely on slow connections/queries
-- Also adds better error handling for mobile network conditions
...
COMMENT ON FUNCTION find_nearby_projects(double precision, double precision, text, integer, double precision) IS
  'Finds the closest projects to given coordinates within max_distance_km radius. Has an 8-second statement timeout to prevent hanging on slow connections. Returns results sorted by geographic distance.';
