-- Fix duplicate projects in employers_list_comprehensive
-- Issue: jsonb_agg was creating duplicate project entries
-- Fix: Use DISTINCT in jsonb_agg and remove redundant SELECT DISTINCT

DROP MATERIALIZED VIEW IF EXISTS employers_list_comprehensive CASCADE;

-- Recreate with fixed aggregation
CREATE MATERIALIZED VIEW employers_list_comprehensive AS
SELECT
  -- Base employer fields (unchanged)
  e.id,
  e.name,
  e.abn,
  e.employer_type,
  ...
  -- Precomputed analytics (unchanged)
  (
  ...
  -- FIXED: Projects with DISTINCT in jsonb_agg
  COALESCE((
  ...
  -- Contractor categories (unchanged)
  COALESCE((
    SELECT jsonb_agg(
      jsonb_build_object(
        'code', category_code,
...
DO $$
BEGIN
  RAISE NOTICE '✅ Fixed duplicate projects issue in employers_list_comprehensive';
  RAISE NOTICE 'Changed: Added DISTINCT to jsonb_agg for projects';
