-- ============================================================================
-- Organizing metrics v2: remove correlated subqueries & rely on aggregated joins
-- Goal: reduce RPC latency (target < 1s cold / < 400ms warm) without changing
--       semantics or permission behaviour.
-- ============================================================================

-- Recreate supporting indexes (idempotent safety for new join patterns)
CREATE INDEX IF NOT EXISTS idx_project_assignments_primary_role
  ON project_assignments(project_id, assignment_type, is_primary_for_role)
  WHERE assignment_type = 'contractor_role';

CREATE INDEX IF NOT EXISTS idx_project_assignments_trade_type
  ON project_assignments(project_id, assignment_type, trade_type_id)
  WHERE assignment_type = 'trade_work';

-- =========================================================================
-- Optimised calculate_organizing_universe_metrics implementation
-- =========================================================================
...
BEGIN
  ...
  ELSIF effective_role = 'lead_organiser' THEN
    patch_ids_filter := NULL; -- lead organisers see all patches
  ELSE
    SELECT ARRAY_AGG(DISTINCT lopa.lead_organiser_id)
    INTO shared_lead_ids
    ...
    WITH organiser_patches AS (
      ...
      WHERE organiser_id = effective_user_id
    ...
    peer_patches AS (
      ...
      WHERE effective_to IS NULL
        ...
        AND lead_organiser_id = ANY(shared_lead_ids)
    )
    SELECT ARRAY_AGG(DISTINCT patch_id)
    INTO patch_ids_filter
    FROM (
  ...
  IF p_patch_ids IS NOT NULL THEN
    SELECT ARRAY_AGG(DISTINCT pid) INTO requested_patch_ids FROM UNNEST(p_patch_ids) AS pid;
  END IF;
  ...
  key_assignments AS (
    ...
    WHERE pa.project_id IN (SELECT id FROM filtered_projects)
      AND (
        ...
        OR (pa.assignment_type = 'trade_work' AND tt.code = ANY(key_contractor_trades))
      )
