-- Migration: Address Search Prioritize Geographic Distance
-- Changes search to sort by geographic distance first, with text match as visual indicator only
-- Increases default results from 5 to 10
...
CREATE OR REPLACE FUNCTION find_nearby_projects(
  ...
  max_results integer DEFAULT 10,  -- Increased from 5 to 10
...
BEGIN
  ...
  WITH project_distances AS (
    SELECT
      ...
      -- This is now just for visual indication, not sorting priority
      CASE
  ...
  ORDER BY pd.distance_km ASC  -- Sort by distance only, removed is_exact_match priority
  LIMIT max_results;
END;
$$;
...
COMMENT ON FUNCTION find_nearby_projects(double precision, double precision, text, integer, double precision) IS
  'Finds the 10 (default) closest projects to given coordinates within max_distance_km radius (default 100km). Returns results sorted by geographic distance. The is_exact_match field indicates if the project address text-matches the search address (for visual indication only).';
