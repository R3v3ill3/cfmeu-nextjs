...
try {
  documentationStructure = JSON.parse(readFileSync(docsPath, 'utf8'))
} catch (error) {
  console.error('Error loading DOCUMENTATION_STRUCTURE.json:', error.message)
  process.exit(1)
}

// Helper function to check if query topics are covered in documentation
function checkDocumentationCoverage(query) {
  const matchedDocs = []
...
module.exports = { testQueries, runAITests, generateQualityReport }