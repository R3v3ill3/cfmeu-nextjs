...
app.get('/v1/projects', async (req, res) => {
    ...
    try {
        ...
        const effectiveSince = sinceParam || profile?.last_seen_projects_at || null;
    ...
    try {
        ...
        if (activeConstructionIds.length > 0) {
          ...
          employers!inner(
            ...
            eba_builders = new Set((activeBuilders || [])
                .filter((b) => Array.isArray(b.employers?.company_eba_records) && b.employers.company_eba_records.length > 0)
                .map((b) => b.employer_id)).size;
            const activeEmployerAssignments = (activeBuilders || []).filter((b) => b.assignment_type === 'employer');
            const uniqueActiveEmployers = new Set(activeEmployerAssignments.map((b) => b.employer_id));
            total_employers = uniqueActiveEmployers.size;
            eba_employers = new Set(activeEmployerAssignments
                .filter((b) => Array.isArray(b.employers?.company_eba_records) && b.employers.company_eba_records.some((eba) => eba.fwc_certified_date))
                .map((b) => b.employer_id)).size;
            const totalsByProject = {};
            (activeBuilders || []).forEach((assignment) => {
                var _a;
                const entry = (totalsByProject[_a = assignment.project_id] || (totalsByProject[_a] = { estimated: [], assigned: [], members: [] }));
                if (typeof assignment.estimated_worker_count === 'number')
                    entry.estimated.push(assignment.estimated_worker_count);
                if (typeof assignment.assigned_worker_count === 'number')
                ...
                if (assignment.delegate_type === 'site_delegate')
                    projectsWithSiteDelegates++;
                if (assignment.delegate_type === 'company_delegate')
                    projectsWithCompanyDelegates++;
                if (assignment.is_hsr)
                    projectsWithHsrs++;
                if (assignment.is_hsr_chair_delegate)
        ...
        if (activePreConstructionIds.length > 0) {
          ...
          employers!inner(
            ...
            const uniqueBuilders = new Set((preConstructionAssignments || [])
                .filter((a) => a.assignment_type === 'contractor_role')
                .map((a) => a.employer_id));
            preConstructionBuilders = uniqueBuilders.size;
            preConstructionEbaBuilders = new Set((preConstructionAssignments || [])
                .filter((a) => a.assignment_type === 'contractor_role' &&
                Array.isArray(a.employers?.company_eba_records) &&
                a.employers.company_eba_records.some((eba) => eba.fwc_certified_date))
                .map((a) => a.employer_id)).size;
            const employerAssignments = (preConstructionAssignments || []).filter((a) => a.assignment_type === 'employer');
            const uniqueEmployers = new Set(employerAssignments.map((a) => a.employer_id));
            preConstructionTotalEmployers = uniqueEmployers.size;
            preConstructionEbaEmployers = new Set(employerAssignments
                .filter((a) => Array.isArray(a.employers?.company_eba_records) &&
                a.employers.company_eba_records.some((eba) => eba.fwc_certified_date))
                .map((a) => a.employer_id)).size;
            const totalsByProject = {};
            (preConstructionAssignments || []).forEach((assignment) => {
                var _a;
                const entry = (totalsByProject[_a = assignment.project_id] || (totalsByProject[_a] = { estimated: [], assigned: [], members: [] }));
                if (typeof assignment.estimated_worker_count === 'number')
                    entry.estimated.push(assignment.estimated_worker_count);
        ...
        const response = {
            ...
            active_pre_construction: {
                ...
                eba_employer_percentage: preConstructionTotalEmployers > 0 ? (preConstructionEbaEmployers / preConstructionTotalEmployers) * 100 : 0,
