...
exports.processMappingSheetScan = processMappingSheetScan;
...
async function processMappingSheetScan(supabase, job) {
    ...
    try {
        ...
        const fileUrl = scanRecord.file_url;
        if (!fileUrl) {
        ...
        const selectedPages = job.payload.selectedPages;
        ...
        const result = await (0, claude_1.extractWithClaude)(pdfBuffer, selectedPages);
        if (!result.success) {
        ...
        await supabase.from('mapping_sheet_scan_costs').insert({
            ...
            model: result.provider === 'claude' ? config_1.config.claudeModel : config_1.config.openaiModel,
