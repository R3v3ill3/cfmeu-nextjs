...
import { IncolinkJobPayload, ScraperJob } from '../types'
...
export async function processIncolinkJob(client: SupabaseClient, job: ScraperJob): Promise<IncolinkSummary> {
  const payload = (job.payload ?? {}) as Partial<IncolinkJobPayload>
  const employerIds = Array.isArray(payload.employerIds) ? payload.employerIds : []

  if (employerIds.length === 0) {
    await appendEvent(client, job.id, 'incolink_no_employers')
  ...
  const employerMap = new Map(
    (employers ?? []).map((row: { id: string; name: string | null; incolink_id: string | null }) => [
      row.id,
  ...
  try {
    for (const [index, employerId] of employerIds.entries()) {
      ...
      if (!employerInfo || !employerInfo.incolinkId) {
        failed += 1
        await appendEvent(client, job.id, 'incolink_employer_missing_id', {
          employerId,
      ...
      await appendEvent(client, job.id, 'incolink_employer_started', {
        employerId,
      ...
      try {
        ...
        await appendEvent(client, job.id, 'incolink_employer_succeeded', {
          employerId,
        ...
        failed += 1
        await appendEvent(client, job.id, 'incolink_employer_failed', {
          employerId,
...
async function fetchMembersFromIncolink(browser: Browser, incolinkNumber: string, invoiceNumber?: string): Promise<InvoiceResult> {
  ...
  try {
    ...
    if (!pwInput) throw new Error('Incolink password input not found')
...
async function detectInvoiceSelection(page: Page): Promise<string | undefined> {
  try {
...
async function clickInvoiceLink(page: Page, invoice: string): Promise<boolean> {
...
async function extractInvoiceDate(page: Page): Promise<string | null> {
  try {
...
async function extractMembers(page: Page): Promise<ParsedMember[]> {
  const frameResults = await Promise.all(
    page.frames().map(async (frame) => {
      try {
        const tableTexts = await frame.$$eval('table tbody tr, table tr', (trs) =>
          trs
            .map((tr) => {
              ...
              const node = (link || span || td) as HTMLElement
              return (node.textContent || '').trim()
            })
            .filter(Boolean)
        )

        const gridTexts = await frame.$$eval('div[role="grid"] div[role="row"]', (rows) =>
          rows
            .map((row) => {
...
): Promise<{ createdWorkers: number; matchedWorkers: number; placementsCreated: number; placementsSkipped: number }> {
