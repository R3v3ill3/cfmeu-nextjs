...
import { withRetry, formatRetryLog, DEFAULT_RETRY_CONFIG, RetryConfig } from '../utils/retry'
...
const FWC_RETRY_CONFIG: Partial<RetryConfig> = {
      ...
      try {
        ...
        for (const candidate of queryCandidates) {
          ...
          const retryResult = await withRetry(
            () => searchFwcAgreements(browser, candidate),
            FWC_RETRY_CONFIG,
            async (retryContext) => {
              // Log retry attempts
              console.warn('[worker] FWC search retry:', formatRetryLog(retryContext, { ...DEFAULT_RETRY_CONFIG, ...FWC_RETRY_CONFIG }))
          ...
          if (retryResult.success && retryResult.data) {
            ...
            if (retryResult.attempts > 1) {
              await appendEvent(client, job.id, 'fwc_search_success_after_retry', {
                employerId,
                employerName,
                query: candidate,
            ...
            await appendEvent(client, job.id, 'fwc_search_retry_exhausted', {
              employerId,
              employerName,
              query: candidate,
              attempts: retryResult.attempts,
        ...
        if (results.length > 0) {
          if (autoLink) {
            ...
            const upsertResult = await withRetry(
              ...
              async (retryContext) => {
                console.warn('[worker] Database upsert retry:', formatRetryLog(retryContext, { ...DEFAULT_RETRY_CONFIG, maxAttempts: 3 }))
                await appendEvent(client, job.id, 'fwc_upsert_retry', {
                  employerId,
                  employerName,
                  attempt: retryContext.attempt,
                  error: retryContext.lastError?.message,
